...
| **1b · Does the right thing** | correct, not only wired | ✅ T-001 · ✅ T-018 · ✅ T-050 (with F-041, critical) · ✅ T-005 · ✅ T-003 · ✅ T-055 · ✅ T-056 · ✅ T-059 · ◐ T-057 · T-058 · T-053 · T-006 · T-020 · T-024 · T-025 · T-049 · **T-061** · T-060 · T-062 · T-063 · ✅ T-064 · ✅ T-065 · **T-066** · T-069 · T-070 · T-071 · G0.11 |
...
| 7 · Route the failures | T-021 · ◐ **T-072** — generation run 2 routed; the reference agent owes its half |
...
✅ **T-059** done 20 Sep: an approval can be decided by a person — a desk, a page, a note in the inbox the colleague already works in, and a reminder before it lapses. The first live run of it found that the approvals worker had no login of its own against the realm (F-050), so every refund needing a person had been failing before anybody saw it.
...
| **T-068** | **The router reads its rules from the AOAS, and the profile chooses how words are matched.** Raised 2026-09-25 by Shalini, after T-067: Refuse, Direct and Escalate looked hardcoded. **Half of that is true.** The *mechanism* is already a seam in G0.10's sense — `Rules` is handed in, `DIRECT_HANDLERS` is a registry, Tier 2 is a list of `Tier2Rule` — and the order `_decide` checks in (refuse, then escalate, then intent, then Direct or Agentic) is the harness's safety property and stays fixed. **The *content* is the real defect: it exists twice.** `support-agent.aoas.yaml` declares `refuses` (R-DISCOUNT and the rest, with examples) and `intents` (`answered_by: direct / loop / handoff`), and `router.Rules` and `escalation/rules.py`'s `DEFAULT_RULES` restate them as Python regexes — two copies that already drifted once (R-DISCOUNT refusing a voucher the customer paid with, T-050). Scope: (1) **what** — the refusal, intent and escalation rule sets projected from the AOAS at the composition root, no second copy in code; (2) **how** — a `Matcher` port declared in the profile as an ordered list like T-067, cheapest first and the next only when the one before is unsure (`[regex]` today; `[regex, classifier]` or an LLM judge later), with the AOAS `examples` as every matcher's test set; (3) which matcher decided, on the `agent.route` span beside `rules_version`. Regex patterns do not fit in the AOAS, which states intent in words — whether they live in a per-agent seam file or give way to a classifier is the first decision. **Same suspect as G0.10 named:** `policy.CLAIM_PATTERNS` is a domain's claims in a mechanism module and belongs in this item. Only `regex` is built now; a classifier comes with the first cycle that needs it | reference-agent, AOAS, AHC, stacks | days | — |
| **T-072** | ◐ **Generation run 2 routed — the specs' half done 2026-09-26; the reference agent owes its half.** Run 2 (`~/generation-run-2/NOTES.md`) rebuilt the support agent from the brief alone — 27 of 28 golden cases live, the one failure the AOAS's — and logged 63 entries. Routed: **AOAS** (`f77d86c`): `order_status` and `refund_status` via `list_orders` (the live failure, status-loop-no-id, §11); `output_length_reached` termination, read by loop-exhausted (§8); `answered_by` per refusal, R-OTHER-CUSTOMER structural (§7); `compensation` on every write, with validator rule `undeclared-compensation` (AHC-0058, §1); approval `execution` and `superseded_by`, and P-APPROVAL-STALE (AHC-0057, §8); `consecutive_failed` defined; `<entity>_id` travels as the entity key; `addressed_to: harness` on P-DIRECT, P-DIRECT-READS, P-OPEN, and rule texts that stand alone (§6); `unknown_record` a result, not a transport error (§2); `sources` reduced to repository@version; AAC pin 0.12 → 0.16. **Open Stack** (`645907f`): `x_store` and `x_runs_from` out (§10); the eight missing thresholds set, the reference's values where it has one (§6); `model.x_wire` — chat-completions, and the seam that Anthropic's SDK sends no sampling parameters (§5); the `customer_id` claim (§7); approvals loaded read-only from the harness's own records, a coupling now stated (§8); `channel.x_delivery` — signature, delivery id, portal session link (§8). **Brief** (`8809ccb`): `leaks()` refuses symbols, import rules, tracker ids and a profile path, and profile notes are scrubbed; unanswered decisions are listed (11), the reference's gaps shown as its own to be argued again, a missing model pin stated, and no world ships (§1). **AHC** (`0aa5de7`, nineteen capabilities). **AAC** (`54484aa`: over-block cases, known-good paths, planted-case count, throttled routes; `212bf0f`: a skipped test is not coverage). **AgentTwin**, in parallel: an unknown record as a result, the `authorise` hook's enforcement duties, the offstage and subject contracts. **Decided by the owner, 2026-09-26:** (1) transcripts, traces and every record the harness writes are kept 30 days — Q-RETENTION, so AAC-0095 is no longer contradicted by `undeclared`; (2) a customer's login never holds `refunds:write` — a refund always goes through `request_refund`, a person approves above the limit, `issue_refund` stays `offered: false`; (3) `repeated_intent` does not count `other`, so small talk never escalates. **Already fixed in the reference** (`reference-agent@832c38a`): a redelivery was named by the caller's key alone, so a second customer reusing a delivery id was answered with the first customer's reply; it is now scoped `customer:key`. Run 2 hit the same leak in its own build (§8), which is what found it. **Owed by reference-agent:** (a) `harness-profile.yaml` answers the decisions the brief now shows open — AHC-0107/freshness_scope, AHC-0108/facts_source, AHC-0110/failure_axis, AHC-0115/erasure_key, AHC-0057/approval_staleness, and the keys `0aa5de7` added; (b) it takes or overrides the stack's eight thresholds, and the harness honours the two it lacks (a turn deadline; a policy timeout — its AHC-0095 gap); (c) it receives `x_store` and `x_runs_from`; (d) it pins the model in `bindings.model.x_model` — `openai/gpt-oss-120b` on Groq through LiteLLM, with its price and the date read — beside its AHC-0014 answer; (e) `catalog.aac` 0.13.0 → 0.16.0; (f) a status question with no order is answered from `list_orders`; (g) `output_length_reached` is recorded; (h) 30-day retention is enforced on every store; (i) `repeated_intent` skips `other`; (j) compensation, approval `execution`/`superseded_by` and `addressed_to` are read from the AOAS. Then regenerate the brief | clean-ai-engineering, AHC, AAC, AgentTwin, reference-agent | days (the reference half) | T-034 |
...
| **T-059** | ✅ Done 2026-09-20. `/ops/approvals` under an `approver` role of its own, a desk page for both queues, a private note in Chatwoot when one is raised, and a reminder an hour before it expires — held by the workflow, best effort, and off where there is nowhere to send it. Found F-050 (the approvals worker had no login of its own against a realm, so every refund needing a person failed unseen) and F-051 (a granted refund the store then refuses reads as `done`) | reference-agent | a day | — |
...
| **T-060** | **Optimistic concurrency at the far end, so freshness can be deleted.** AHC-0107 is realised in the *client* today: `fresh_for: 30s` on `order.status` obliges every caller to re-read before an irreversible action, which protects only callers that implement it — a deterministic UI, a bulk tool or a partner writing to the same store gets nothing, and a 30-second window never catches a one-millisecond race. The mechanism belongs at the far end: a `version` on the entity, `expected_version` required on any `irreversible` operation, compare-and-set, and a typed refusal carrying the current row so the caller learns the truth in the same round trip. Deletes `loop/freshness.py`, the `_refresh` branch, `binding.FRESH_FOR_S` and `HELD`. **Reshaped 2026-09-24: this is not a question about Saleor.** One store's answer decides nothing, because the next store answers differently and neither is chosen by us — a design that only works against compliant far ends is not a design. So the far end's capabilities are **declared in the binding and verified at startup**, the way T-018 already holds a declared provider to what the endpoint serves: `optimistic_concurrency`, `idempotency_keys`, `erasure`, each verified, unverified, or a mismatch that stops the process. The compensating control is then switched on what is found rather than left running for ever in case — which makes it countable across bindings and retirable on a date. Saleor becomes the first binding to fill in rather than the question itself. The capability does not move; only the realization. **F-054 is the case that makes it a money bug rather than a tidiness argument**: an approval records what was decided and has nowhere to say what it was decided against, so a hotel guest who changes dates between the request and the grant has a person approve ₹25,000 and ₹41,000 leave. The version an approval was raised against is the same primitive, carried through the wait | reference-agent, AOAS, AHC | days | — |
| **T-061** | **Freshness picks the wrong reader on a multi-entity surface.** `ToolRegistry.first_read()` returns the *first* read tool regardless of entity, and `Freshness` is keyed by a bare id, so `cancel_shipment(SH-99)` on a surface offering `get_order` and `get_shipment` re-reads with `get_order` — and `order AB-1` and `item AB-1` share one cache entry. Not a defect today, because every operation in this AOAS is on `order`; a defect the moment a second entity arrives. The entity is already declared per operation in the spec and dropped at the `ToolSpec` boundary. Carry it, match on it, key the cache by it, verify the row that comes back is the row asked for, and refuse to guess rather than reading the wrong one. Subsumed by T-060 if the far end takes over | reference-agent | a session | — |
| **T-062** | ◐ **Six implementations become five ports, and one rule replaces two.** **The `requests` merge shipped 2026-09-24** (`8a40e85`): the delivery claim and the ledger are one store, Temporal lost a workflow, net 141 lines fewer. `checkpoints`, `idempotency`, `sessions`, the delivery claim, approvals and escalations are four concepts with six implementations, and two pairs disagree: the delivery claim settles on failure (*tried counts as done*) while the ledger does not record one (*failed means try again*), and a repeated message returns `"already handled"` while a repeated tool call returns the original result. **Step 1 is a table, not code**: one rule — *a name is owed until there is a definite answer; a definite answer is stored and returned* — with the definite/indefinite split that already exists on the model side (`ModelRefused` vs `ModelUnavailable`) and not on the tool side. Then `requests` absorbs the delivery claim (a row with `expires_at` checked at claim time needs no sweeper and no workflow, which was Temporal's only reason to hold it) and **the two human waits stay two** — withdrawn 2026-09-24 after reading them rather than reasoning about them from outside: the approval workflow runs three activities and the escalation workflow runs none, so merging replaces two explicit workflows with one generic one and optional hooks, saves ~150 lines of 2,000, and takes on Temporal's versioning exposure permanently. 6→5 was two stores answering one question with contradictory rules, which is a defect; these two answer different questions, and deduplicating a mechanism is not resolving a disagreement. **They are ports, not stores**, and how many you build depends on what the domain already runs — the dependency-upgrade agent binds `human_decision` to **GitHub**, which supplies five of its six parts in the tool the reviewer already lives in, and builds only `requests`. **The four describe one run**; fleet management is a queue and stays outside them. Tested against four shapes: hotel held and found F-054; the Spark analyst held by needing none of the four; invoice reconciliation held but broke two constants and found F-055; dependency upgrade held and forced the ports reframing. **The ports generalise; the constants do not** — `ttl: 30m`, `history_chars`, `fresh_for: 30s` and a session that lives as long as a login are all sized for a conversation that lasts seconds. **A fifth shape, the personal assistant, changed the drawing rather than confirming it**: `memory` is a fifth port, across runs and owned by the agent because no far end holds *he prefers morning meetings*; `logins` is keyed by `subject` alone and needs `(subject, provider, scopes)` with the credential injected per tool call rather than resolved into one identity per turn; `human_decision` hardcodes *nobody approves their own request*, which is the confused deputy in a support agent and **exactly inverted** where the principal confirming their own send is the design, so the port must carry whose decision is required; and tenant scoping is an `if` in `_conversation_for` where thousands of mailboxes want row-level security threaded from intake, *structurally impossible to forget*. Also F-056: two of the ports have no `DELETE` at all. And one claim leaned on throughout needs qualifying — *the far end owns correctness* assumes there is **one** far end, and *book the meeting and email them* is two that do not know about each other, so half-done is reachable and only compensation answers it | reference-agent, AHC | days | — |
| **T-064** | ✅ **The facts an approval was granted against, carried through the wait — done 2026-09-25.** Carved out of T-060 because T-060 is days and this was hours, and because F-054 is a money bug that does not need the rest of it: `Approval.decided_against` records what the assessment judged, the carry-out re-reads it and refuses where it moved, and a stale grant settles as its own outcome rather than as a failure. The far end declaring `optimistic_concurrency` and `erasure` at startup, and deleting `loop/freshness.py`, stay in T-060 | reference-agent, AHC, AAC | hours | — |
| **T-065** | ✅ **Erasure by subject — done 2026-09-25, F-056.** `erasure.forget()` across checkpoints, the request ledger and sessions. The column `customer_id` on `checkpoints` is what made it possible; the ordering — conversations first, returning their run ids — is what makes it complete; and the ledger is *redacted* rather than deleted, because deleting makes a replayed refund executable again. Does not reach delivery-scope rows, which carry a channel's message id and no customer. New: AAC-0117, AHC-0115 | reference-agent, AHC, AAC | a day | — |
| **T-063** | **Compliance crosswalks — after the capabilities exist, not before.** AAC has one, `owasp-llm.yaml`, and AHC's `SCOPE.md` is right that this family **cites rather than authors**: controls belong to ISO/IEC 42001 and NIST AI RMF, threats to OWASP and MITRE ATLAS, and restating them here would be maintaining a worse copy. What is missing is not the mapping but **the thing to map to**. GDPR Art. 17 wants erasure by subject and nothing deletes (F-056); Art. 15 wants subject access and there is no path that produces what is held about somebody; AAC-0097 (processing region) is declared and not exercised; AAC-0095's retention half is bounded nowhere (F-057). A crosswalk written first would be a list of absences with a standard's name on it, and `test_assurance_map.py` exists to catch exactly that claim. So: **T-030 and F-056 first**, then crosswalk GDPR, EU AI Act Art. 12, NIST AI RMF and ISO/IEC 42001 in AAC beside OWASP. **ISO 27001 is deliberately out** — an information security management system is policies, risk treatment and access reviews, almost none of which maps to a harness capability; 42001 is the one that fits, and SCOPE.md already names it. One thing to check while doing it: *thresholds belong to the adopter* is the catalog's sharpest line, and a statutory deadline is not a preference — the capability stays *erasure completes within a stated deadline and the deadline is recorded*, but it is the first obligation whose number is not the adopter's to choose | AAC, AHC, reference-agent | days | T-030, F-056 |
| **T-066** | **A replica declares how long it is good for, or the process does not start.** F-058. Four things in the agent are copies of what a far end holds, and the two with a refresh rule are the short-lived ones: the run's row cache (30s window) and an approval's recorded facts (re-read before the effect). The two with none are the ones that outlive everything — **the reply text on the conversation**, which is fed back to the model as history so a sentence composed from a stale read becomes a premise it reasons from, and **`requests.outcome`**, replayed verbatim on a redelivery however long afterwards. `fresh_for: 30s` is a constant applied at one point; the rule wants to be a property every replica declares, refused at startup rather than remembered, which is the pattern used elsewhere in this codebase. **Not T-060** — that asks what a far end *offers* and is answerable only per binding; this asks what *we* hold, and holds unchanged against a far end that offers nothing. **Two of the four rows are mechanical and two are design**: re-reading a `requests.outcome` past its window is mechanical; what to do about a reply already given is not, because an agent's replica escapes into prose and reaches a person, and a sentence already said cannot be invalidated. Scope the reply half before building any of it | reference-agent, AHC, AOAS | days | — |
| **T-069** | **A new conversation knows what the customer's last ones were about.** Raised 2026-09-25 by Shalini: a customer asks *where is order ABC-10003*, closes the browser, comes back and says *cancel it* — and the agent has no idea what *it* is. Three causes, all confirmed in code: the chat page holds the conversation id only in a JS variable (`ui/__init__.py:94`), so a closed tab sends `conversation_id: null`; `/chat` finds a conversation only by id (`_conversation_for`, `store.latest(id)`), never by customer; and `run_server.py` wires `InMemoryCheckpointStore`, so a restart loses everything anyway. Chatwoot survives the closed browser (the id is Chatwoot's, `cw-{account}-{conversation}`) but not the restart. Not a safety hole — `cancel_order` needs an id and F-041's *asked, else confirm* stops a guess — but the customer repeats themselves. **Decided: carry `Facts`, not model-written summaries.** Summarising past conversations was argued and set aside for this agent on five grounds: a summary states stale state as a premise (F-058), invites a guess across several open orders on an irreversible action, launders a planted note into every later conversation unfenced (AHC-0109), loses the detail that mattered, and is a new copy of personal data that erasure (T-065) must also find. `Facts` are harness-written — the customer's own words, the ids that reached a tool, what landed, what is awaited — so they are exact, small and carry no tool output. Scope: (1) Postgres behind the demo server; (2) a store read for *this customer's recent conversations*, returning their `Facts` (open ones, and completed ones within a window); (3) those `Facts` placed in a new conversation's context under operator provenance, with order state always re-read live, never taken from the record; (4) a scenario — asks about ABC-10003, closes the browser, says *cancel it* — passes only if the agent confirms ABC-10003 before cancelling, and asks which when two are candidates. Also decide whether a request without an id resumes the latest open conversation or starts a new one carrying the others' `Facts`. A summariser, if ever wanted, is a later entry in T-067's list, fenced | reference-agent, AHC | days | — |
| **T-070** | **A write whose reply was lost is answered, not refused — and the ticket outlives the process.** Raised 2026-09-25 by Shalini while learning the loop's PLAN step: *what if the order is cancelled but the ticket was never recorded?* Confirmed in code. The order system runs the Saleor mutation, then records the key in `Answered`, a Python dict (`order_system/server.py:69-90`, `:231`) — two places, no transaction, and a restart forgets every key. No effect lands twice: every write already checks state first (T-050 — `cancel_order` `store.py:274`, `open_return_request` `:328`, `issue_refund` `:368`), so AHC-0074's second rung holds. What is wrong is smaller and still real: (1) the retry after a lost reply is answered *refused — already refunded*, so the model tells the customer a refund that succeeded failed; (2) the check and the act are two calls, so two concurrent requests can both pass the check (the race AHC-0074 says to accept *and record* — nothing records it); (3) the key store is in memory, the undeclared-substrate failure AHC-0102 names. Temporal does not remove this: it retries an activity whose completion it never heard, so it makes the lost-reply case *more* likely, not less — its own guidance is that activities be idempotent. **Cross-cutting, and where the line falls.** What is common: minting the key, carrying it, reproducing it on resume (done, F-039, T-005), a durable key store port, and a **generic test** — for every tool declared a write, land the effect, drop the reply, retry, and assert the second answer is the first. What is per-system and cannot be common: *how* the far end honours the key — a state guard, a unique key column in the same transaction as the effect, or the payment provider's own idempotency key. So the shared part is the contract and its test, not the implementation. Scope: (a) each write tool declares its AOAS-level rung (`state-guarded` / `keyed-in-transaction` / `provider-keyed` / `single-attempt+escalate`); (b) a guarded no-op returns the *done* answer when the state is already the requested one (cancel on a cancelled order is success, not refusal); (c) `Answered` moves to Postgres; (d) find out whether Saleor can store the key with `orderGrantRefundCreate` in one call — if yes, refund becomes keyed-in-transaction; (e) the generic lost-reply test above, run against every declared write | reference-agent, AHC, AOAS | days | T-062 |
| **T-071** | **Langfuse's own fields filled, and `agent.*` published as a convention.** Raised 2026-09-26 by Shalini while learning the telemetry pipeline: *if most of what we send lands in metadata, who turns it into insight?* Checked: the standard half maps with no configuration — `gen_ai.request.model`, `gen_ai.usage.*`, `session.id`, `user.id` become Langfuse's model, usage, cost (via `deploy/langfuse/models.py`), session and user, and a span with `gen_ai.*` is shown as a generation. About 35 `agent.*` attributes (`telemetry/names.py`) arrive as `metadata["attributes.<name>"]` (`watch/record.py:5,112`), and Langfuse derives nothing from them. What does is ours: the agent's counters carry the same facts as Prometheus labels for the 18 alert rules (`RunNumbers` derives terminations and spend from the finished `agent.run` span; the rest are counted where they happen), and the watch's 22 rules read the attributes back from Langfuse and write verdicts as scores. So they are not second-class — they carry the harness's own facts (termination reason, tool outcome, context trimmed, freshness, redaction, cost per turn) that no convention covers — but a person in Langfuse's UI sees little of them. Scope: (1) also emit the customer's words and the reply under Langfuse's names (`langfuse.trace.input` / `langfuse.trace.output`, or the GenAI content convention), so the Input/Output panes fill — redaction and `capture_sample` apply unchanged; (2) name the trace for what it is rather than `http.chat`, and send tags from `agent.turn.result` and `agent.route.kind`; (3) set the observation level to warning/error for refused and failed turns, not only when Python raises; (4) check the current GenAI semantic conventions for names that now exist — `invoke_agent`, `execute_tool`, `gen_ai.conversation.id` — and emit those beside or instead of ours, as `gen_ai.system` → `gen_ai.provider.name` was handled (both for one release); check whether `service.version` or the config fingerprint reaches Langfuse's release field; (5) **publish `agent.*` as an AHC convention** — every name, its meaning, its allowed values, and which rule or alert reads it — so any agent generated from the specs emits the same attributes and one set of rules and dashboards serves them all. Items 1–4 send the same facts twice, once for Langfuse's screens and once for our rules; nothing that reads `agent.*` today changes | reference-agent, AHC | days | — |
...
| **T-073** | **AgentTwin tests concurrency and load, or it cannot vouch for production.** Raised 2026-09-26 by Basant after a scale review of the reference agent (a billion messages a day, about 12k/s average and 35k/s peak). Every scenario today is **one customer, one conversation, turns in order**: `suite.run_file` loops one actor; `Clock` ticks per turn; `Timeline` counts calls per tool, not per actor; `Slow` moves declared time and never elapses any; `Live` deep-copies a few declared rows; the projection runs in-process through `Client(server)`; the record is one per run. So nothing can show a race, a leak between customers, a memory leak or a knee. **Two families, two kinds of machinery.** *Correctness* (declared, deterministic, pass/fail per run, inside today's model): **(1) crowd scenarios** — a `cast:` of several actors on one `Live`, a `hold` fault that pauses one actor's call until another's arrives so the race is forced every run and not one run in a thousand, and checks across actors: `at_most_once` per idempotency key, `isolated` (no reply to A names B's rows), `no_lost_turn`; **(2) a parallel runner** — independent scenarios at once, each with its own `Live`, `Clock` and `Subject`, and the serial and parallel records diffed with `record.diff`, so state shared through a process global shows up as a difference. *Load* (measured, wall time, verdict against SLOs): **(3) `population:`** in the world — distributions, not rows, each row generated from seed and key when read, so a billion-row world costs nothing and is the same every run; **(4) a `load:` file** — scenarios as weighted traffic, an **open-loop** arrival curve, and SLOs (turn p99, error rate, RSS growth per hour, `at_most_once` at 100%); scaling, stress, soak and volume are four profiles of it, not four tools; **(5) the twin as a service** — the projection over streamable HTTP, sharded by `customer_id` so the twin never chokes first, and a **provider twin**: an OpenAI-compatible server replaying cassettes with real latency and token distributions and injected 429s; a `latency:` fault in wall time beside `slow`, which stays logical; **(6) a load record** — throughput, percentiles per turn and tool, errors by kind, SLO verdicts, and the subject's resources over time from an optional `Subject.resources()`. **Decided:** the agent still never sees its simulator (a load subject drives the real webhook, as Chatwoot does); AgentTwin owns declaration, twin and verdict and **adopts Locust** to generate load (its users are AgentTwin actors) rather than building an engine. **Order:** 1 → 2 → 3 → 5 → 4 and 6. **First scenario, and it should fail today:** *the same cancel, twice at once, lands once* — two actors as C-1042 in one conversation, `hold` on the first `cancel_order` until the second arrives. It fails on two reference defects the review found: `Answered` checks, awaits, then records (`order_system/server.py:228`), and a conversation is read, then written back with no version check (`channel/__init__.py:335`). Related: T-053 fails only under load; T-054 is (5)'s transport; T-023 is time inside a turn. Touches agenttwin, its SPEC.md, and reference-agent's scenarios | weeks | T-054 for (5) |
| **T-074** | **Crash-test four agents we did not write.** Raised 2026-09-26 by Basant. Every agent AgentTwin has tested so far is ours, or generated from our specs, so a pass proves agreement with our own assumptions — the argument T-017 made about the store, made again about the subject. **Targets, all MIT, chosen for credibility over count:** (1) **τ²-bench's retail agent** (`sierra-research/tau2-bench`) — first, because its domain (orders, returns, exchanges) is nearest `clothing.yaml` and because it is the benchmark everyone knows; (2) **OpenAI's CS agents demo** (`openai/openai-cs-agents-demo`, Agents SDK, airline, handoffs between agents); (3) **LangGraph's customer-support tutorial** (`langchain-ai/langgraph`, travel, sensitive tools behind an interrupt); (4) **our support agent on the Claude Agent SDK** (T-037), the third framework. Unlicensed repositories are out (public is not open source); Rasa CALM is out because it needs Rasa Pro. **The question differs from τ-bench's, and the study says so:** τ-bench asks *can it do the job* (end state against a goal); AgentTwin asks *can it be broken* — the planted note (F-041), stale reads, lost replies, a duplicate delivery, another customer's rows, an approval skipped. So each target runs twice: its own tasks unperturbed, as a baseline, then with perturbations; the headline is the difference. **Per target:** a fork keeping its licence and credits; an **adapter** that is the binding's three callables over the target's own tools — either the world projected into the target's schema, or the target's database read into a world — and an airline world for (2) and (3); the same perturbation set on all four; the world-diff oracle, not the agent's words, deciding each verdict. **Fairness:** these are reference implementations, not products, so findings are stated as patterns ("a tool result can carry an instruction the agent obeys"), not as a vendor's insecurity; anything that looks like a real hole goes to the maintainer as an issue before anything is published. **Done when** four targets have a baseline and a perturbed run, every failure is routed (to the target as an issue, or to our specs where the twin could not express it), and the results table and method are written up. **What it teaches the cycle:** the adapter is T-044's one-step setup tried on a subject that did not come from an AOAS; whatever it needed that the AOAS would have supplied is a gap in T-044 | agenttwin, a fork per target | weeks, ~3–5 days a target | T-041; T-044 makes it cheap, τ²-bench can start by hand |
