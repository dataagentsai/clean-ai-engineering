...
| **T-062** | **Six stores become four, and one rule replaces three.** `checkpoints`, `idempotency`, `sessions`, the delivery claim, approvals and escalations are four concepts with six implementations, and two pairs disagree with each other: the delivery claim settles on failure (*tried counts as done*) while the ledger does not record one (*failed means try again*), and a repeated message returns `"already handled"` while a repeated tool call returns the original result. **Step 1 is a table, not code**: one rule — *a name is owed until there is a definite answer; a definite answer is stored and returned* — with a definite/indefinite split that already exists on the model side (`ModelRefused` vs `ModelUnavailable`) and not on the tool side. Then `requests` absorbs the delivery claim (a row with `expires_at` checked at claim time needs no sweeper and no workflow, which was Temporal's only reason to hold it), and `human_decision` absorbs both waits. Tested against three shapes: hotel held and found F-054; the Spark analyst held by needing none of them; invoice reconciliation held but broke two assumptions — a session cannot outlive a run that waits days (F-055), and `ttl: 30m` is a constant where it should be a function of the row. **The stores generalise; the constants do not** | reference-agent, AHC | days | — |
